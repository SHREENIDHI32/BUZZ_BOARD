...
const SummarizeArticle = () => {
  ...
  const handleSummarize = async () => {
    ...
    try {
      // Send a POST request to the backend to summarize the content
      const response = await axios.post('http://localhost:3000/summarize', {
        articleContent: articleContent,
      });
  
      // Check if the response contains a successful summary
      if (response.data.success && response.data.summary_text) {
        setSummary(response.data.summary_text); // Display the summarized content
      } else {
