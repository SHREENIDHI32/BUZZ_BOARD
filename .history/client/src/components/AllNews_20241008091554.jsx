...
function AllNews() {
  ...
  function handlePrev() {
    setPage(prev => Math.max(prev - 1, 1)); // Prevent going below 1
  }
  ...
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    console.log(`Fetching data for page: ${page}`); // Log current page
    ...
    fetch(`https://news-aggregator-dusky.vercel.app/all-news?page=${page}&pageSize=${pageSize}`)
      ...
      .then(myJson => {
        console.log('Response JSON:', myJson); // Log the response
